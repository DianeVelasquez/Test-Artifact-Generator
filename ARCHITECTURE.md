...
| AnalyzerAgent | Detects technology profile, uses deterministic extractors when available, and falls back to generic LLM-based extraction. |
...
The product intentionally extracts high-confidence facts with code before asking an LLM to infer anything.

| Evidence | Deterministic Result |
|----------|----------------------|
| Spring Boot controller mappings | Operation names, HTTP methods, relative paths, and controller source evidence. |
| Spring Boot build and application files | `spring-boot-rest` technology profile signals. |
| REST endpoint without existing `.feature` files | Starter success and validation BDD scenarios. |

The LLM remains useful for semantic enrichment: service descriptions, business language, legacy BDD extraction, and ambiguous repositories.
...
- End-to-end tests should be added around the Spring Boot fixture once a test runner is introduced.
